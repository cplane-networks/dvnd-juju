...
def neutron_agent_show(args):
    ...
    cmd = "su - ubuntu -c 'source nova.rc && neutron agent-show {}'".format(agent_id)
    res = commands.getoutput(cmd)
    action_set({'result-map.message': res})
...
def create_ogr_zone(args):
    ...
    cmd = "su - ubuntu -c 'source nova.rc && nova aggregate-create {} {}'".format(aggr_name, avail_zone)
    commands.getoutput(cmd)
    cmd = "su - ubuntu -c 'source nova.rc && nova aggregate-add-host {} {}'".format(aggr_name, ogr_compute)
    commands.getoutput(cmd)

    cmd = "su - ubuntu -c 'source nova.rc && nova aggregate-details {}'".format(aggr_name)
    res = commands.getoutput(cmd)
    action_set({'result-map.message': res})
...
def delete_ogr_zone(args):
    ...
    cmd = "su - ubuntu -c 'source nova.rc && nova aggregate-remove-host {} {}'".format(aggr_name, ogr_compute)
    res = commands.getoutput(cmd)
    cmd = "su - ubuntu -c 'source nova.rc && nova aggregate-delete {}'".format(aggr_name)
    res = commands.getoutput(cmd)
    action_set({'result-map.message': res})
...
--no-gateway --allocation-pool start={},end={}'".format(sub_name, net_name, cidr, pool_start, pool_end)
    res = commands.getoutput(cmd)
    action_set({'result-map.message': res})
...
--visibility public --container-format bare --disk-format qcow2 < {}'".format(name, image_path)
    res = commands.getoutput(cmd)
    action_set({'result-map.message': res})
